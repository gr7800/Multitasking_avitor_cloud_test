...
const Startgame = () => {
    ...
    useEffect(() => {
        if (minutes < 0) {
            ...
            let mscore = JSON.parse(localStorage.getItem("muserscore"));
            
            localStorage.setItem("muserscore",JSON.stringify({...mscore,[difficulty]:Math.floor(avragepercentage)}));
            navigate("/result",{
    ...
    return (
        <Box bg={'gray.200'}>
            ...
            <Box maxH={"100vh"} bg={"aquamarine"} display={"flex"} flexDirection={"column"} rowGap={"50px"} border={"10px solid #ccc"} width={{ base: "100%", md: "50%" }} margin={"auto"} borderRadius={"15px"} padding={"20px"}>
